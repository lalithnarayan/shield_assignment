...
import { format, isToday, compareAsc, eachDayOfInterval, addDays, differenceInCalendarDays, formatDate } from "date-fns";
...
const DateCell: React.FC<DateCellProps> = ({ day, panelMonth }) => {
  ...
  const previewClasses = inPreviewRange && !isSelected(day)
    ? "border border-dotted border-blue-400 bg-blue-400/20"
    : "";
    ...
    switch (selected.length) {
      ...
      case 2:
        ...
        if (compareAsc(day, selected[0]) < 0) {
          ...
          deselectRange();
          selectRange(day, selected[1], true);
        } else if (compareAsc(day, selected[1]) > 0) {
          // Clicked day is after the last selected date
          if (compareAsc(day, addDays(selected[0], maxRangeDays - 1)) > 0) {
            toast(`Cannot select more than ${maxRangeDays} days.`);
            return;
          }
          deselectRange();
